...
// fix: hanya query DB unread jika user login, cache di local var
...
.sb-header {
  ...
  padding:.9rem 1rem .8rem; border-bottom:1px solid var(--sb-border); flex-shrink:0;
}
.sb-brand {
  display:flex; align-items:center; gap:.6rem;
  text-decoration:none; overflow:hidden; flex:1; min-width:0;
}
.sb-brand-logo { width:30px; height:30px; flex-shrink:0; object-fit:contain; }
.sb-brand-icon {
  width:30px; height:30px; flex-shrink:0;
  background:rgba(255,255,255,.15); border-radius:7px;
  display:flex; align-items:center; justify-content:center; color:var(--sb-accent);
}
.sb-brand-text {
  font-size:13.5px; font-weight:800; color:#fff;
  white-space:nowrap; overflow:hidden; text-overflow:ellipsis; letter-spacing:-.01em;
}
.sb-brand-sub { font-size:10px; color:var(--sb-text-muted); white-space:nowrap; overflow:hidden; text-overflow:ellipsis; line-height:1.2; }
.sb-collapse-btn {
  width:26px; height:26px; flex-shrink:0;
  background:rgba(255,255,255,.1); border:1px solid var(--sb-border);
...
.sb-section-label {
  ...
  padding:.75rem 1rem .25rem; white-space:nowrap; overflow:hidden;
  transition:opacity var(--sb-transition);
}
.sb-sidebar.collapsed .sb-section-label { opacity:0; }
.sb-item {
  display:flex; align-items:center; gap:.6rem;
  padding:.48rem .9rem; margin:.1rem .5rem;
...
.sb-item:hover { background:var(--sb-bg-item); color:#fff; }
.sb-item.active { background:var(--sb-bg-active); color:#fff; }
.sb-item.active::before {
  content:''; position:absolute; left:0; top:20%; bottom:20%;
  width:3px; background:var(--sb-accent);
  border-radius:0 3px 3px 0; margin-left:-.5rem;
}
.sb-item-icon { width:18px; height:18px; flex-shrink:0; display:flex; align-items:center; justify-content:center; }
.sb-item-label { flex:1; overflow:hidden; text-overflow:ellipsis; transition:opacity var(--sb-transition),width var(--sb-transition); }
...
.sb-item-badge {
  font-size:10px; font-weight:800; background:#e53e3e; color:#fff;
  padding:.1em .45em; border-radius:10px; min-width:18px;
  text-align:center; flex-shrink:0; transition:opacity var(--sb-transition);
}
.sb-item-chevron {
  width:14px; height:14px; flex-shrink:0; color:var(--sb-text-muted);
  transition:transform var(--sb-transition),opacity var(--sb-transition);
}
.sb-group.open > .sb-group-toggle .sb-item-chevron { transform:rotate(90deg); }
.sb-group-children { overflow:hidden; max-height:0; transition:max-height var(--sb-transition); }
.sb-group.open > .sb-group-children { max-height:400px; }
.sb-group-children .sb-item { padding-left:2.5rem; font-size:12.5px; font-weight:500; }
.sb-sidebar.collapsed .sb-group-children { display:none; }
.sb-divider { height:1px; background:var(--sb-border); margin:.4rem .5rem; }
.sb-sidebar.collapsed .sb-item:hover::after {
  content:attr(data-label); position:absolute;
  left:calc(var(--sb-w-col) + 6px); top:50%; transform:translateY(-50%);
  background:#1C1714; color:#fff; font-size:12px; font-weight:600;
  padding:.3em .7em; border-radius:6px; white-space:nowrap;
  z-index:300; pointer-events:none; box-shadow:0 2px 12px rgba(0,0,0,.25);
}
.sb-footer { border-top:1px solid var(--sb-border); padding:.65rem .5rem; flex-shrink:0; }
.sb-user {
  display:flex; align-items:center; gap:.65rem;
  padding:.4rem .5rem; border-radius:var(--sb-radius);
  cursor:pointer; text-decoration:none;
  transition:background var(--sb-transition); overflow:hidden; position:relative;
}
...
.sb-avatar {
  ...
  background:var(--sb-accent); border:1.5px solid rgba(255,255,255,.25);
}
...
.sb-user-role { font-size:10.5px; color:var(--sb-text-muted); white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
...
.sb-topbar {
  height:var(--tb-h); background:var(--tb-bg);
  border-bottom:1px solid var(--tb-border);
  display:flex; align-items:center; padding:0 1.25rem; gap:.75rem;
  flex-shrink:0; position:sticky; top:0; z-index:100;
  box-shadow:0 1px 4px rgba(28,23,20,.06);
}
.sb-topbar-title { flex:1; font-size:14px; font-weight:800; color:#1C1714; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
.sb-topbar-right { display:flex; align-items:center; gap:.5rem; }
.sb-topbar-btn {
  width:34px; height:34px; border-radius:8px; border:1.5px solid #EDE8DE;
  background:#fff; cursor:pointer;
  display:flex; align-items:center; justify-content:center;
  color:#6B6055; position:relative; transition:all 200ms;
}
...
.sb-notif-dropdown {
  position:fixed;
  top:calc(var(--tb-h) + 6px); right:1.25rem;
  width:340px; background:#fff;
  border:1.5px solid #EDE8DE; border-radius:12px;
...
.sb-notif-hdr {
  display:flex; align-items:center; justify-content:space-between;
  padding:.75rem 1rem; border-bottom:1px solid #EDE8DE;
}
...
.sb-notif-footer { padding:.55rem 1rem; border-top:1px solid #EDE8DE; text-align:center; }
.sb-notif-footer a { font-size:12px; font-weight:700; color:#7B1C1C; text-decoration:none; }
.sb-notif-footer a:hover { text-decoration:underline; }
.sb-user-dropdown {
  position:fixed; top:calc(var(--tb-h) + 6px); right:1.25rem;
...
.sb-user-dropdown-header { padding:.65rem 1rem; border-bottom:1px solid #EDE8DE; background:#FBF8F3; }
...
.sb-overlay { display:none; position:fixed; inset:0; z-index:190; background:rgba(0,0,0,.45); }
.sb-overlay.open { display:block; }
@media (max-width:767.98px) {
  .sb-sidebar { transform:translateX(-100%); width:var(--sb-w) !important; }
...
<aside class="sb-sidebar" id="sbSidebar">
  ...
  <nav class="sb-nav">
    ...
    <a href="<?= $baseUrl ?>/" class="sb-item <?= sbActive('/', $currentUri) ? 'active' : '' ?>" data-label="Dashboard">
      <span class="sb-item-icon"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg></span>
      <span class="sb-item-label">Dashboard</span>
    </a>

    <a href="<?= $baseUrl ?>/meetings" class="sb-item <?= sbActive('/meetings', $currentUri) ? 'active' : '' ?>" data-label="Kegiatan">
      <span class="sb-item-icon"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg></span>
      <span class="sb-item-label">Kegiatan</span>
    </a>

    <a href="<?= $baseUrl ?>/tindak-lanjut" class="sb-item <?= sbActive('/tindak-lanjut', $currentUri) ? 'active' : '' ?>" data-label="Tindak Lanjut">
      <span class="sb-item-icon"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="9 11 12 14 22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg></span>
      <span class="sb-item-label">Tindak Lanjut</span>
    </a>

    <a href="<?= $baseUrl ?>/dokumen" class="sb-item <?= sbActive('/dokumen', $currentUri) ? 'active' : '' ?>" data-label="Dokumen">
      <span class="sb-item-icon"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/></svg></span>
      <span class="sb-item-label">Dokumen</span>
    </a>

    <a href="<?= $baseUrl ?>/notifications" class="sb-item <?= sbActive('/notifications', $currentUri) ? 'active' : '' ?>" data-label="Notifikasi">
      <span class="sb-item-icon"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg></span>
    ...
    <div class="sb-group <?= $adminActive ? 'open' : '' ?>" id="sbGroupAdmin">
      <div class="sb-item sb-group-toggle <?= $adminActive ? 'active' : '' ?>" data-label="Administrasi" onclick="toggleSbGroup('sbGroupAdmin')">
        <span class="sb-item-icon"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 4.68 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 2.83-2.83l.06.06A1.65 1.65 0 0 0 9 4.68a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg></span>
      ...
      <div class="sb-group-children">
        <?php if ($isAdmin): ?>
        <a href="<?= $baseUrl ?>/users" class="sb-item <?= sbActive('/users', $currentUri) ? 'active' : '' ?>" data-label="Pengguna">
          <span class="sb-item-icon"><svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg></span>
          <span class="sb-item-label">Pengguna</span>
        </a>
        <a href="<?= $baseUrl ?>/roles" class="sb-item <?= sbActive('/roles', $currentUri) ? 'active' : '' ?>" data-label="Role & Permission">
          <span class="sb-item-icon"><svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg></span>
          <span class="sb-item-label">Role &amp; Permission</span>
        </a>
        <a href="<?= $baseUrl ?>/departments" class="sb-item <?= sbActive('/departments', $currentUri) ? 'active' : '' ?>" data-label="Unit Kerja">
          <span class="sb-item-icon"><svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg></span>
          <span class="sb-item-label">Unit Kerja</span>
        </a>
        <a href="<?= $baseUrl ?>/recurring" class="sb-item <?= sbActive('/recurring', $currentUri) ? 'active' : '' ?>" data-label="Recurring">
          <span class="sb-item-icon"><svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 0 1 4-4h14"/><polyline points="7 23 3 19 7 15"/><path d="M21 13v2a4 4 0 0 1-4 4H3"/></svg></span>
          <span class="sb-item-label">Recurring</span>
        </a>
        <?php endif; ?>
        <a href="<?= $baseUrl ?>/notulen-templates" class="sb-item <?= sbActive('/notulen-templates', $currentUri) ? 'active' : '' ?>" data-label="Template Notulen">
          <span class="sb-item-icon"><svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg></span>
          <span class="sb-item-label">Template Notulen</span>
        </a>
        <?php if ($isAdmin): ?>
        <a href="<?= $baseUrl ?>/admin/activity-log" class="sb-item <?= sbActive('/admin/activity-log', $currentUri) ? 'active' : '' ?>" data-label="Log Aktivitas">
          <span class="sb-item-icon"><svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><polyline points="10 9 9 9 8 9"/></svg></span>
          <span class="sb-item-label">Log Aktivitas</span>
        </a>
        <a href="<?= $baseUrl ?>/settings" class="sb-item <?= sbActive('/settings', $currentUri) ? 'active' : '' ?>" data-label="Pengaturan">
          <span class="sb-item-icon"><svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 4.68 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 2.83-2.83l.06.06A1.65 1.65 0 0 0 9 4.68a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg></span>
...
<div class="sb-main" id="sbMain">
  <div class="sb-topbar">
    ...
    <div class="sb-topbar-right">
      <button class="sb-topbar-btn" id="sbNotifBtn" title="Notifikasi">
        ...
        <span class="sb-topbar-notif-badge" id="notifDot" style="<?= $unread > 0 ? '' : 'display:none;' ?>"></span>
      </button>
  ...
  <div class="sb-notif-dropdown" id="sbNotifDropdown">
    ...
    <div class="sb-notif-list" id="notif-list" style="max-height:340px;overflow-y:auto;">
      <div style="text-align:center;padding:1.5rem;color:#A89E90;font-size:13px;">
...
<?php
// base.php menutup .sb-main dan .sb-layout setelah $content
?>
...
(function () {
  ...
  /* ── Collapse state ────────────────────────────────────────── */
  if (localStorage.getItem(LS_COL) === '1') sidebar.classList.add('collapsed');
  ...
  if (titleEl) {
    titleEl.textContent = activeItem
      ? (activeItem.dataset.label || activeItem.querySelector('.sb-item-label')?.textContent?.trim() || '')
  ...
  async function loadNotifs() {
    ...
    try {
      ...
      if (!d.data || d.data.length === 0) {
        list.innerHTML = '<div style="text-align:center;padding:1.5rem;color:#A89E90;font-size:13px;">Tidak ada notifikasi baru</div>';
      } else {
        list.innerHTML = d.data.map(n => {
          // fix #1: gunakan n.url (bukan n.link) sesuai skema DB
          const href    = n.url || '#';
          // fix #2: is_read sudah di-cast ke int di server, tapi tetap cek parseInt
          const isRead  = parseInt(n.is_read) === 1;
          const time    = n.created_at_human || '';  // fix #6: field sekarang tersedia
          return `
            <a href="${href}"
               class="sb-notif-item"
               style="display:flex;flex-direction:column;gap:.2rem;padding:.65rem 1rem;
                      border-bottom:1px solid #F5F0E8;text-decoration:none;
                      background:${isRead ? '#fff' : '#FFF8F8'};transition:background 150ms;"
               onmouseenter="this.style.background='#F5F0E8'"
               onmouseleave="this.style.background='${isRead ? '#fff' : '#FFF8F8'}'"
               data-id="${n.id}" data-read="${n.is_read}">
              <div style="font-size:12.5px;font-weight:${isRead ? '500' : '800'};color:#1C1714;line-height:1.4;">
                ${n.message}
              </div>
              <div style="font-size:11px;color:#A89E90;">${time}</div>
            </a>
          `;
        }).join('');
      }
      ...
      // Update dot badge
      if (notifDot) notifDot.style.display = (d.unread_count > 0) ? '' : 'none';
    } catch(e) {
      document.getElementById('notif-list').innerHTML =
        '<div style="padding:1rem;color:#e53e3e;font-size:12.5px;">Gagal memuat notifikasi.</div>';
      notifLoaded = false; // allow retry
    }
  }

  notifBtn.addEventListener('click', () => {
    positionDropdown('sbNotifBtn', 'sbNotifDropdown');
  ...
  /* fix #3: markAllRead — kirim body {all:true} + Content-Type + CSRF */
  document.getElementById('sbMarkAllRead')?.addEventListener('click', async function () {
    this.disabled = true;
    try {
      ...
      // fix #5: reset notifLoaded agar dropdown di-reload segar berikutnya
      notifLoaded = false;
      if (notifDot) notifDot.style.display = 'none';
      // Update tampilan item di dropdown yang sudah terbuka
      document.querySelectorAll('#notif-list .sb-notif-item').forEach(el => {
        el.style.background    = '#fff';
        el.dataset.read        = '1';
        const msg = el.querySelector('div:first-child');
        if (msg) msg.style.fontWeight = '500';
      });
    } catch(err) {}
    this.disabled = false;
  });
  ...
  document.getElementById('sbUserBtn')?.addEventListener('click', () => {
    positionDropdown('sbUserBtn', 'sbUserDropdown');
