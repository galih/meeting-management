...
// Stat card config
$statCards = [
    ['key' => 'total',       'label' => 'Total Tugas',    'color' => 'blue',   'icon' => '📋'],
    ['key' => 'pending',     'label' => 'Pending',         'color' => 'yellow', 'icon' => '⏳'],
    ['key' => 'in_progress', 'label' => 'Dikerjakan',      'color' => 'orange', 'icon' => '🔄'],
    ['key' => 'done',        'label' => 'Selesai',         'color' => 'green',  'icon' => '✅'],
    ['key' => 'overdue',     'label' => 'Terlambat',       'color' => 'red',    'icon' => '⚠️'],
];

// PHP 7.4 compat: array lookup
...
$statusLabel    = ['pending' => 'Pending', 'in_progress' => 'In Progress', 'done' => 'Done', 'cancelled' => 'Cancelled'];

// Query params string (untuk pagination link)
$qp = array_filter([
    'q'        => $search   ?? '',
    'status'   => $status   ?? '',
    'priority' => $priority ?? '',
...
// Kanban: kelompokkan data per kolom
$kanbanCols = [
    'pending'     => ['label' => 'Pending',         'color' => 'yellow', 'icon' => '⏳', 'items' => []],
    'in_progress' => ['label' => 'Dikerjakan',       'color' => 'blue',   'icon' => '🔄', 'items' => []],
...
$allUsersJson = json_encode(array_values(array_map(function ($u) {
...
<!-- ── Toolbar ──────────────────────────────────────────────────── -->
<div class="card mb-3">
  <div class="card-header flex-column flex-md-row gap-2 align-items-start align-items-md-center">
    <form method="GET" action="<?= $baseUrl ?>/tindak-lanjut"
          class="d-flex flex-wrap gap-2 flex-fill align-items-center">
      <input type="text" name="q" value="<?= htmlspecialchars($search ?? '') ?>"
             class="form-control form-control-sm" style="min-width:180px;max-width:240px;"
             placeholder="Cari deskripsi...">

      <select name="status" class="form-select form-select-sm" style="width:145px;" id="filter-status">
    ...
    <div class="d-flex gap-2 flex-shrink-0 align-items-center">
      ...
      <?php if ($isAdminLike): ?>
      <button class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#modalTambahTL">
        ...
        <tbody>
          ...
          <tr class="<?= $overdue ? 'table-danger' : '' ?>" id="trow-<?= $tl['id'] ?>">
            <td style="max-width:280px;">
              <div class="fw-semibold text-truncate"><?= htmlspecialchars($tl['description']) ?></div>
              <?php if ($overdue): ?>
              <span class="badge bg-red-lt text-red" style="font-size:10px;">⚠ Terlambat</span>
              <?php endif; ?>
            </td>
            <td>
              <a href="<?= $baseUrl ?>/meetings/<?= $tl['meeting_id'] ?>"
                 class="text-orange small text-decoration-none">
                <?= htmlspecialchars(mb_strimwidth($tl['meeting_title'], 0, 36, '…')) ?>
              </a>
            </td>
            <td class="text-muted small"><?= htmlspecialchars($tl['assignee_name'] ?? '-') ?></td>
            <td class="text-muted small">
              <?= !empty($tl['due_date']) ? date('d M Y', strtotime($tl['due_date'])) : '-' ?>
            </td>
            <td>
              <span class="badge bg-<?= $pc ?>-lt"><?= $priorityLabel[$tl['priority']] ?? ucfirst($tl['priority']) ?></span>
            </td>
            <td>
              <?php if ($canEdit): ?>
              ...
              <?php else: ?>
              <span class="badge bg-<?= $sc ?>"><?= $statusLabel[$tl['status']] ?? ucfirst($tl['status']) ?></span>
              <?php endif; ?>
            </td>
            <td class="text-end">
              <div class="d-flex gap-1 justify-content-end align-items-center">
                <button class="btn btn-sm btn-ghost-secondary btn-notes position-relative"
                  ...
                  💬<?php if ($nc > 0): ?><span class="badge bg-blue ms-1 note-badge" id="nbadge-<?= $tl['id'] ?>"><?= $nc ?></span><?php else: ?><span class="note-badge ms-1" id="nbadge-<?= $tl['id'] ?>"></span><?php endif; ?>
                </button>
                <?php if ($isAdminLike): ?>
...
<div id="view-kanban" style="display:none;">
  ...
  <div class="alert alert-info py-2 small mb-3">
    ℹ️ Filter status <strong><?= htmlspecialchars($statusLabel[$status] ?? $status) ?></strong> aktif
    — kanban menampilkan semua kolom.
  </div>
  <?php endif; ?>
    ...
    <div class="col-12 col-md-6 col-xl-3">
      <div class="card h-100">
        <div class="card-header py-2 bg-<?= $col['color'] ?>-lt">
          <h5 class="card-title mb-0 d-flex align-items-center gap-1">
            <?= $col['icon'] ?> <?= $col['label'] ?>
        ...
        <div class="card-body p-2 kanban-col"
          ...
          <div class="kanban-empty text-center text-muted py-4 small"
               style="border:2px dashed #dee2e6;border-radius:8px;">Tidak ada tugas</div>
          <?php endif; ?>
          ...
          <div class="kanban-card card card-sm mb-2 <?= $overdue ? 'border-danger' : '' ?>"
               ...
               style="cursor:<?= ($canEdit && $isAdminLike) ? 'grab' : 'default' ?>;
                      <?= in_array($tl['status'], ['done', 'cancelled']) ? 'opacity:.7' : '' ?>">
            <div class="card-body p-2">
              <div class="d-flex justify-content-between align-items-start gap-1 mb-1">
                <span class="small fw-semibold flex-fill" style="line-height:1.3;">
                  <?= htmlspecialchars($tl['description']) ?>
                </span>
                <span class="badge bg-<?= $pc ?>-lt text-<?= $pc ?> flex-shrink-0" style="font-size:9px;">
              ...
              <div class="d-flex justify-content-between align-items-center mt-1">
                <a href="<?= $baseUrl ?>/meetings/<?= $tl['meeting_id'] ?>"
                   class="text-orange text-decoration-none" style="font-size:10px;">
                  📋 <?= htmlspecialchars(mb_strimwidth($tl['meeting_title'], 0, 26, '…')) ?>
                </a>
    ...
    <div class="modal-content">
      <form id="formTambahTL">
        ...
        <div class="modal-body">
          ...
          <div class="row g-3">
            <div class="col-12">
              ...
              <select name="meeting_id" id="tl-meeting-id" class="form-select" required>
                <option value="">-- Pilih Meeting --</option>
                <?php
                $meetings = Database::query(
                    "SELECT id, title FROM meetings ORDER BY start_datetime DESC LIMIT 200"
                );
            ...
            <div class="col-12">
              ...
              <textarea name="description" id="tl-desc" class="form-control" rows="3"
        ...
        <div class="modal-footer">
          ...
          <button type="submit" class="btn btn-primary ms-auto" id="btn-save-tl">
            <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="16" height="16"
                 viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <div>
          ...
          <div class="text-muted small" id="notes-desc" style="max-width:340px;"></div>
        </div>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
      <div class="modal-body p-0">
        ...
        <div class="border-top p-3" style="position:relative;">
          ...
          <div class="text-muted mt-1" style="font-size:10px;">
            Ketik <code>@nama</code> untuk mention · <kbd>Ctrl+Enter</kbd> kirim
          </div>
        </div>
      </div>
    </div>
...
function updateNoteBadge(tlId, count) {
  document.querySelectorAll('#nbadge-' + tlId).forEach(function (el) {
    if (count > 0) {
      el.className  = 'badge bg-blue note-badge' + (el.closest('#view-table') ? ' ms-1' : '');
      el.textContent = count;
    } else {
      el.className  = 'note-badge' + (el.closest('#view-table') ? ' ms-1' : '');
      el.textContent = '';
    }
  });
}
...
function bindDelButtons() {
  document.querySelectorAll('.btn-del').forEach(function (btn) {
    ...
    btn.addEventListener('click', async function () {
      ...
      if (trow)  trow.remove();
      if (kcard) {
        var col = kcard.closest('.kanban-col');
        kcard.remove();
        updateColCount(col);
      }
    });
...
function updateColCount(colEl) {
  ...
  if (count === 0 && !empty) {
    ...
    e.className  = 'kanban-empty text-center text-muted py-4 small';
    e.style.cssText = 'border:2px dashed #dee2e6;border-radius:8px;';
    e.textContent   = 'Tidak ada tugas';
    colEl.appendChild(e);
...
document.querySelectorAll('.kanban-col').forEach(function (col) {
  Sortable.create(col, {
    ...
    onEnd: async function (evt) {
      var card      = evt.item;
      var newColEl  = evt.to;
      var newStatus = newColEl.dataset.status;
...
if (formTambah) {
  formTambah.addEventListener('submit', async function (e) {
    ...
    btn.disabled = true;
    var fd  = new FormData(formTambah);
    var obj = {};
    fd.forEach(function (v, k) { obj[k] = v; });
    var res = await fetch(BASE + '/tindak-lanjut', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (d.success) {
      ...
      formTambah.reset();
      // Reload halaman agar data baru muncul di tabel & kanban
      window.location.reload();
    } else {
      alertEl.textContent = d.message || 'Gagal menyimpan tindak lanjut.';
...
function renderNote(n) {
  ...
  var delBtn = n.can_delete
    ...
    : '';
  var noteHtml = escHtml(n.note).replace(/@([\w]+(?:\s[\w]+)*)/g,
    '<span class="badge bg-azure-lt text-azure">@$1</span>');
  div.innerHTML =
...
if (btnMarkDone) {
  btnMarkDone.addEventListener('click', async function () {
    ...
    if (trow) {
      var sel = trow.querySelector('.status-select');
...
var btnSendNote = document.getElementById('btn-send-note');
if (btnSendNote) {
  btnSendNote.addEventListener('click', async function () {
...
if (noteInput) {
  noteInput.addEventListener('keydown', function (e) {
    ...
    if (e.ctrlKey && e.key === 'Enter') {
      hideMentionDropdown();
      if (btnSendNote) btnSendNote.click();
    }
  });
}
...
if (noteInput) {
  noteInput.addEventListener('input', function () {
    ...
    var results    = ALL_USERS.filter(function (u) {
