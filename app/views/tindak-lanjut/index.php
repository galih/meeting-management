    ...
    <div class="card">
      <div class="card-body">
        <div class="d-flex align-items-center">
          <div class="subheader text-muted"><?= $sc['label'] ?></div>
        </div>
...
<div class="card">
  <div class="card-header flex-column flex-md-row gap-2">
    <form method="GET" action="/tindak-lanjut" class="d-flex flex-wrap gap-2 flex-fill">
      ...
      <a href="/tindak-lanjut" class="btn btn-sm btn-ghost-danger">Reset</a>
      <?php endif; ?>
    </form>
  </div>
    ...
    <table class="table table-vcenter card-table table-hover">
      ...
      <tbody>
        ...
        <?php foreach ($tindakLanjutList as $tl):
          ...
          $canEdit  = Auth::can('admin','sekretaris')
        ...
        <tr class="<?= $overdue?'table-danger':'' ?>">
          ...
          <td>
            <a href="/meetings/<?= $tl['meeting_id'] ?>" class="text-orange small">
              <?= htmlspecialchars($tl['meeting_title']) ?>
            </a>
          </td>
          <td class="text-muted"><?= htmlspecialchars($tl['assigned_name'] ?? '-') ?></td>
          ...
          <td>
            <?php if ($canEdit): ?>
            <select class="form-select form-select-sm status-select" data-id="<?= $tl['id'] ?>" style="width:130px;">
              <?php foreach (['pending'=>'Pending','in_progress'=>'In Progress','done'=>'Done','cancelled'=>'Cancelled'] as $v=>$l): ?>
          ...
          <td>
            ...
            <button class="btn btn-sm btn-ghost-danger btn-del" data-id="<?= $tl['id'] ?>" title="Batalkan">✕</button>
            <?php endif; ?>
          </td>
        </tr>
        <?php endforeach; ?>
...
document.querySelectorAll('.status-select').forEach(sel => {
  sel.addEventListener('change', async function () {
    const res = await fetch(`/tindak-lanjut/${this.dataset.id}/status`, {
      method:  'POST',
      headers: {'Content-Type':'application/json'},
      body:    JSON.stringify({ status: this.value })
    });
    const d = await res.json();
    if (!d.success) alert(d.message || 'Gagal update status');
    else if (this.value === 'done') this.closest('tr').style.opacity = '0.5';
  });
});

document.querySelectorAll('.btn-del').forEach(btn => {
  btn.addEventListener('click', async function () {
    if (!confirm('Batalkan tindak lanjut ini?')) return;
    const res = await fetch(`/tindak-lanjut/${this.dataset.id}/delete`, { method:'POST' });
    const d   = await res.json();
