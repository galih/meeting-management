...
$statusBg = [
  'scheduled' => 'rgba(47,107,196,.14)',
  'ongoing'   => 'rgba(201,168,76,.22)',
  'done'      => 'rgba(26,115,64,.14)',
  'cancelled' => 'rgba(123,28,28,.14)',
];
$statusColor = [
  'scheduled' => '#1557a0',
  'ongoing'   => '#7a5000',
...
$pStatusBg    = ['accepted'=>'rgba(26,115,64,.12)','invited'=>'rgba(47,107,196,.12)','declined'=>'rgba(123,28,28,.12)','attended'=>'rgba(13,122,138,.12)','pending'=>'rgba(122,106,90,.10)'];
$pStatusColor = ['accepted'=>'#1a7340','invited'=>'#1557a0','declined'=>'#7B1C1C','attended'=>'#0a6a78','pending'=>'#7A6A5A'];
$pStatusLabel = ['accepted'=>'Diterima','invited'=>'Diundang','declined'=>'Ditolak','attended'=>'Hadir','pending'=>'Menunggu'];
$prioBg       = ['high'=>'rgba(123,28,28,.12)','medium'=>'rgba(201,168,76,.14)','low'=>'rgba(26,115,64,.12)'];
$prioColor    = ['high'=>'#7B1C1C','medium'=>'#7a5000','low'=>'#1a7340'];
$prioLabel    = ['high'=>'Tinggi','medium'=>'Sedang','low'=>'Rendah'];
$tlsBg        = ['pending'=>'rgba(122,106,90,.10)','in_progress'=>'rgba(47,107,196,.12)','done'=>'rgba(26,115,64,.12)','cancelled'=>'rgba(123,28,28,.12)'];
$tlsColor     = ['pending'=>'#7A6A5A','in_progress'=>'#1557a0','done'=>'#1a7340','cancelled'=>'#7B1C1C'];
...
$mBg         = $statusBg[$mStatus]         ?? 'rgba(122,106,90,.10)';
$mColor      = $statusColor[$mStatus]      ?? '#7A6A5A';
...
<?php /* ================================================================
   HERO
================================================================ */ ?>
<div class="sh-hero mb-3" style="--mc:<?= $brand ?>">
  ...
  <div class="sh-hero-body">
    <div class="sh-hero-row">
      ...
      <div class="sh-hero-left">
        ...
        <div class="sh-meta">
          <span class="sh-badge" style="background:<?= $mBg ?>;color:<?= $mColor ?>">
            <?= $mIcon ?>&nbsp;<?= $mLabel ?>
          </span>
    ...
    <span class="sh-strip-item">
      ...
      <?php if ($isLink): ?>
        <a href="<?= htmlspecialchars($loc) ?>" target="_blank" rel="noopener noreferrer" class="sh-strip-link">
          Buka Link
          <svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>
        </a>
      <?php else: ?>
...
<div class="row g-3">
  ...
  <div class="col-xl-3 col-lg-4">
    ...
    <div class="sh-stats mb-3">
      ...
      <div class="sh-stat">
        <div class="sh-stat-ico" style="background:rgba(47,107,196,.10);color:#2F6BC4">
      ...
      <div class="sh-stat">
        <div class="sh-stat-ico" style="background:var(--brand-light);color:var(--brand)">
      ...
      <div class="sh-stat">
        <div class="sh-stat-ico" style="background:rgba(26,115,64,.10);color:#1a7340">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
        </div>
        <div>
          <div class="sh-stat-val" style="color:#1a7340"><?= $doneTL ?></div>
          <div class="sh-stat-lbl">Selesai</div>
        </div>
      </div>

      <div class="sh-stat">
        <div class="sh-stat-ico" style="background:<?= $overdueTL > 0 ? 'rgba(123,28,28,.10);color:#7B1C1C' : 'rgba(122,106,90,.08);color:#7A6A5A' ?>">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
        </div>
        <div>
          <div class="sh-stat-val" style="<?= $overdueTL > 0 ? 'color:#7B1C1C' : '' ?>"><?= $overdueTL ?></div>
    ...
    <div class="sh-card">
      ...
      <div class="sh-card-bd sh-doc-list">
        ...
        <button type="button" class="sh-doc-btn sh-doc-teal" id="btnKirimRingkasan">
          <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="22 2 11 13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>
          Kirim Ringkasan
        </button>
        <?php endif; ?>
        <?php endif; ?>
  ...
  <div class="col-xl-9 col-lg-8">
    <div class="sh-main">
      ...
      <div id="sh-panel-tl" class="sh-panel active" role="tabpanel" aria-labelledby="sh-tab-tl">
        ...
        <div class="sh-empty">
          <div class="sh-empty-ico">
            <svg width="34" height="34" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.2"><polyline points="9 11 12 14 22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg>
          </div>
        ...
        <div class="table-responsive">
          <table class="sh-table" aria-label="Daftar Tindak Lanjut">
            ...
            <?php foreach ($tindakLanjutList as $tl):
              ...
              $pBg  = $prioBg[$tl['priority'] ?? '']    ?? 'rgba(122,106,90,.10)';
              $pClr = $prioColor[$tl['priority'] ?? ''] ?? '#7A6A5A';
              $pLbl = $prioLabel[$tl['priority'] ?? ''] ?? ucfirst($tl['priority'] ?? '-');
              $sBg  = $tlsBg[$tl['status'] ?? '']       ?? 'rgba(122,106,90,.10)';
              $sClr = $tlsColor[$tl['status'] ?? '']    ?? '#7A6A5A';
            ...
            ?>
              <tr class="<?= $isOver ? 'row-overdue' : '' ?>">
                <td>
                  ...
                  <?php if ($isOver): ?>
                  <span class="sh-bdg" style="background:rgba(123,28,28,.10);color:#7B1C1C;font-size:10px;margin-top:.2rem;display:inline-flex">Terlambat</span>
                  <?php endif; ?>
                </td>
                <td>
                  ...
                  <?php else: ?><span style="color:var(--text-muted)">&mdash;</span><?php endif; ?>
                </td>
                <td>
                  <span style="font-size:12.5px;<?= $isOver ? 'color:#7B1C1C;font-weight:700' : 'color:var(--text-muted)' ?>">
                    <?= !empty($tl['due_date']) ? date('d M Y', strtotime($tl['due_date'])) : '&mdash;' ?>
                  </span>
                </td>
                <td><span class="sh-bdg" style="background:<?= $pBg ?>;color:<?= $pClr ?>"><?= $pLbl ?></span></td>
                <td><span class="sh-bdg" style="background:<?= $sBg ?>;color:<?= $sClr ?>"><?= $sLbl ?></span></td>
                <td>
      ...
      <div id="sh-panel-peserta" class="sh-panel" role="tabpanel" aria-labelledby="sh-tab-peserta">
        ...
        <div class="sh-empty">
          <div class="sh-empty-ico">
            <svg width="34" height="34" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75"/></svg>
          </div>
        ...
        <div class="sh-peserta-grid">
          <?php foreach ($participants as $p):
            $psBg  = $pStatusBg[$p['status'] ?? '']    ?? 'rgba(122,106,90,.10)';
            $psClr = $pStatusColor[$p['status'] ?? ''] ?? '#7A6A5A';
          ...
          <div class="sh-peserta-card">
            ...
            <span class="sh-bdg" style="background:<?= $psBg ?>;color:<?= $psClr ?>"><?= $psLbl ?></span>
          </div>
          <?php endforeach; ?>
        </div>
        <?php endif; ?>
    ...
    <div class="modal-content">
      <div class="modal-body text-center py-4">
        <div class="sh-del-ico mb-3">
          <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/></svg>
        </div>
    ...
    btnSaveTL.addEventListener('click', function () {
      ...
      .then(function (d) {
        if (d.success) { window.location.reload(); }
          ...
          shToast(d.message || 'Gagal menyimpan.', 'error');
...
.sh-toast--err { background:var(--brand); }
...
.sh-hero-body { padding:1.3rem 1.6rem 1rem; }
...
.sh-title     { font-size:clamp(15px,2.2vw,21px); font-weight:800; color:#fff; margin:0; letter-spacing:-.02em; line-height:1.25; }
.sh-meta      { display:flex; flex-wrap:wrap; align-items:center; gap:.35rem; margin-top:.5rem; }
.sh-badge     { display:inline-flex; align-items:center; gap:.3rem; font-size:11px; font-weight:700; padding:.25em .65em; border-radius:20px; white-space:nowrap; }
...
/* Hero actions */
.sh-actions   { display:flex; flex-wrap:wrap; gap:.4rem; align-items:flex-start; padding-top:1.15rem; }
.sh-btn       { display:inline-flex; align-items:center; gap:.35rem; font-size:12.5px; font-weight:700; padding:.38rem .9rem; border-radius:8px; cursor:pointer; border:none; text-decoration:none; transition:background .18s; white-space:nowrap; }
...
.sh-btn-danger{ background:rgba(192,57,43,.22); border:1.5px solid rgba(255,255,255,.22); color:#ffd5cf; }
.sh-btn-danger:hover { background:rgba(192,57,43,.38); color:#fff; }

/* Hero strip */
.sh-strip     { display:flex; flex-wrap:wrap; align-items:center; gap:.5rem; background:rgba(0,0,0,.20); padding:.5rem 1.6rem; font-size:11.5px; color:rgba(255,255,255,.82); }
.sh-strip-item{ display:flex; align-items:center; gap:.3rem; }
.sh-strip-sep { opacity:.4; }
...
.sh-card-hd { display:flex; align-items:center; gap:.35rem; font-size:11px; font-weight:800; text-transform:uppercase; letter-spacing:.07em; color:var(--text-muted); padding:.6rem .9rem; border-bottom:1px solid var(--border-light); background:#faf6ef; }
...
/* Main card & tabs */
.sh-main  { background:var(--bg-card); border:1px solid var(--border-light); border-radius:12px; overflow:hidden; }
.sh-tabs  { display:flex; border-bottom:1px solid var(--border-light); background:#faf6ef; padding:0 1rem; }
.sh-tab   { display:inline-flex; align-items:center; gap:.35rem; font-size:13px; font-weight:600; color:var(--text-muted); padding:.7rem .9rem; background:none; border:none; border-bottom:2.5px solid transparent; cursor:pointer; transition:color .15s,border-color .15s; white-space:nowrap; }
.sh-tab:hover  { color:var(--brand); }
.sh-tab.active { color:var(--brand); border-bottom-color:var(--brand); font-weight:700; }
.sh-pill  { display:inline-flex; align-items:center; justify-content:center; min-width:18px; height:18px; padding:0 5px; font-size:10px; font-weight:700; background:var(--border-light); color:var(--text-muted); border-radius:9px; }
.sh-tab.active .sh-pill { background:var(--brand-light); color:var(--brand); }

.sh-panel        { display:none; }
.sh-panel.active { display:block; }

.sh-toolbar { padding:.7rem 1rem; border-bottom:1px solid var(--border-light); background:#fdfaf5; }
...
.sh-empty-ico { width:52px; height:52px; border-radius:14px; background:var(--brand-xlight); display:flex; align-items:center; justify-content:center; color:var(--brand); margin-bottom:.85rem; }
.sh-empty p   { font-size:13.5px; margin-bottom:.9rem; }

.sh-add-btn { display:inline-flex; align-items:center; gap:.35rem; font-size:12.5px; font-weight:700; background:var(--brand); color:#fff; padding:.4rem 1rem; border-radius:8px; border:none; cursor:pointer; transition:background .18s; }
.sh-add-btn:hover { background:var(--brand-dark); }

/* Table */
.sh-table { width:100%; border-collapse:collapse; font-size:13px; color:var(--text-main); }
.sh-table thead th { font-size:10.5px; font-weight:700; text-transform:uppercase; letter-spacing:.06em; color:var(--text-muted); background:#faf6ef; border-bottom:2px solid var(--border); padding:.6rem 1rem; white-space:nowrap; }
...
.row-overdue       { background:rgba(123,28,28,.025) !important; }
.row-overdue:hover { background:rgba(123,28,28,.05) !important; }
...
.sh-av       { display:inline-flex; align-items:center; justify-content:center; width:30px; height:30px; border-radius:50%; font-size:12px; font-weight:800; color:#fff; flex-shrink:0; }
.sh-av-sm    { width:22px; height:22px; font-size:10px; }
.sh-link     { display:inline-flex; align-items:center; gap:.25rem; font-size:12px; color:var(--brand); font-weight:600; text-decoration:none; }
.sh-link:hover { color:var(--brand-dark); text-decoration:underline; }

/* Peserta grid */
.sh-peserta-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(210px,1fr)); gap:.5rem; padding:.85rem 1rem; }
...
/* Delete icon */
.sh-del-ico { width:58px; height:58px; border-radius:50%; background:rgba(123,28,28,.10); color:#7B1C1C; display:flex; align-items:center; justify-content:center; margin:0 auto; }

/* Responsive */
@media (max-width:575px) {
  .sh-hero-body { padding:1rem; }
  .sh-strip     { padding:.45rem 1rem; }
  .sh-actions   { padding-top:0; }
