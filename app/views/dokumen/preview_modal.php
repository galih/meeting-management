...
.dm-preview-close-btn {
  ...
  transition:background 150ms; z-index:10; font-size:18px; line-height:1;
}
...
.dm-preview-content pre   {
  ...
  overflow:auto; max-height:calc(100vh - 80px); font-size:13px;
  max-width:calc(100vw - 360px); white-space:pre-wrap; word-break:break-all;
}
.dm-preview-nopreview { display:flex; flex-direction:column; align-items:center; gap:1rem; color:#fff; text-align:center; }
...
.dm-preview-panel {
  width:300px; flex-shrink:0; background:#fff; overflow-y:auto;
  display:flex; flex-direction:column; box-shadow:-4px 0 20px rgba(0,0,0,.2);
}
.dm-preview-panel-header {
  padding:1.1rem 1.1rem .75rem; border-bottom:1px solid #F0EBE2;
...
.dm-panel-title    { flex:1; min-width:0; }
.dm-panel-filename {
...
.dm-panel-section { padding:.85rem 1.1rem; border-bottom:1px solid #F5F0E8; }
.dm-panel-section-title { font-size:10.5px; font-weight:800; color:#A89E90; text-transform:uppercase; letter-spacing:.06em; margin-bottom:.6rem; }
.dm-panel-row { display:flex; justify-content:space-between; align-items:center; font-size:12.5px; color:#4A5568; margin-bottom:.35rem; }
.dm-panel-row:last-child { margin-bottom:0; }
.dm-panel-row strong { color:#1C1714; font-weight:700; }
.dm-panel-actions { padding:1rem 1.1rem; display:flex; flex-direction:column; gap:.45rem; margin-top:auto; }
.dm-panel-btn {
  display:flex; align-items:center; justify-content:center; gap:.5rem;
  height:38px; border-radius:9px; font-size:13px; font-weight:700;
  border:1.5px solid transparent; cursor:pointer; width:100%; transition:all 180ms;
}
...
.dm-panel-share-item {
  ...
  font-size:12px; color:#4A5568; padding:.3rem 0; border-bottom:1px solid #F5F0E8;
}
.dm-panel-share-item:last-child { border-bottom:none; }
.dm-panel-share-avatar {
  width:24px; height:24px; border-radius:50%; background:#7B1C1C; color:#fff;
  display:flex; align-items:center; justify-content:center;
  font-size:10px; font-weight:800; flex-shrink:0;
}
.dm-preview-spinner { display:flex; align-items:center; justify-content:center; height:100%; color:#fff; font-size:14px; }
...
(function(){
  ...
  window.openPreview = function(fileId) {
    ...
    document.getElementById('preview-content').innerHTML = '<div class="dm-preview-spinner">Memuat pratinjau...</div>';
    document.getElementById('preview-panel').innerHTML   = '<div style="padding:1.5rem;color:#A89E90;font-size:13px">Memuat info...</div>';
  ...
  async function loadPreviewContent(fileId) {
    ...
    try {
      ...
      if (f.mime_type.startsWith('image/'))      box.innerHTML = '<img src="'+url+'" alt="'+escHtml(f.original_name)+'" loading="lazy">';
      else if (f.mime_type.startsWith('video/')) box.innerHTML = '<video controls autoplay><source src="'+url+'" type="'+escHtml(f.mime_type)+'">Browser tidak mendukung video.</video>';
      else if (f.mime_type.startsWith('audio/')) box.innerHTML = '<audio controls autoplay style="width:360px"><source src="'+url+'" type="'+escHtml(f.mime_type)+'">Browser tidak mendukung audio.</audio>';
      else if (f.mime_type === 'application/pdf') box.innerHTML = '<iframe src="'+url+'#toolbar=1&navpanes=0" title="'+escHtml(f.original_name)+'"></iframe>';
      else if (f.mime_type === 'text/plain' || f.mime_type === 'text/csv') {
        const text = await (await fetch(url)).text();
        box.innerHTML = '<pre>'+escHtml(text.slice(0,50000))+'</pre>';
      } else box.innerHTML = errBox('Tipe file tidak dikenali.');
    } catch(e) { box.innerHTML = errBox('Gagal memuat pratinjau.'); }
  }
  ...
  function renderPanel(f, shares) {
    const panel = document.getElementById('preview-panel');
    panel.innerHTML = '';
    panel.appendChild(buildPanelDOM(f, shares));
  ...
  function buildPanelDOM(f, shares) {
    ...
    const tgl = f.created_at ? new Date(f.updated_at||f.created_at).toLocaleDateString('id-ID',{day:'2-digit',month:'short',year:'numeric'}) : '-';
    det.innerHTML =
    ...
    let shtml = '<div class="dm-panel-section-title">Dibagikan ke</div>';
    if (!shares||!shares.length) shtml += '<span style="font-size:12px;color:#A89E90">Belum dibagikan.</span>';
    else shtml += shares.map(s=>
      ...
      +'<span style="font-size:11px;color:#A89E90;background:#F5F0E8;border-radius:4px;padding:.1em .45em">'+(s.permission==='download'?'DL':'View')+'</span>'
      +'</div>'
    ).join('');
    sh.innerHTML = shtml;
    frag.appendChild(sh);
  ...
  document.getElementById('modal-preview').addEventListener('click', e => {
    if (e.target===document.getElementById('modal-preview') || e.target===document.getElementById('preview-main')) closePreview();
  });
