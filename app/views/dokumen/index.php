...
<div class="dm-wrap">
  ...
  <main class="dm-main">
    ...
    <!-- Folder grid (hanya di section my-files) -->
    ...
    <div class="dm-folder-grid" id="folder-grid">
      ...
      <a href="<?= $base ?>/dokumen?folder=<?= $folder['id'] ?>" class="dm-folder-card" data-folder-id="<?= $folder['id'] ?>">
        ...
        <div style="flex:1;min-width:0">
          ...
          <div class="dm-folder-meta"><?= $folder['file_count'] ?> file &bull; <?= DokumenModel::formatSize((int)$folder['total_size']) ?></div>
        </div>
        <?php if ($canUpload): ?>
    ...
    <div class="dm-empty">
      ...
      <p><?= $search ? 'Tidak ada file yang cocok dengan pencarian.' : 'Belum ada file di sini.' ?></p>
      <?php if ($canUpload && !$search): ?>
  ...
  <div class="dm-modal">
    ...
    <div class="dm-modal-body">
      <div class="dm-dropzone" id="upload-dropzone">
        <input type="file" id="input-file-upload" multiple style="display:none" accept="*">
        ...
        <small>Maks 50 MB/file &bull; PDF, Word, Excel, PPT, Gambar, Video, ZIP</small>
      </div>
    ...
    <div class="dm-modal-footer">
      ...
      <button class="dm-btn" id="btn-do-upload"
              style="background:#7B1C1C;color:#fff;border-color:#5A1212" disabled>
...
<div class="dm-modal-overlay" id="modal-folder">
  <div class="dm-modal">
    ...
    <div class="dm-modal-footer">
      ...
      <button class="dm-btn" id="btn-do-folder"
              style="background:#7B1C1C;color:#fff;border-color:#5A1212"
...
<!-- ======================== MODAL: RENAME FILE ======================== -->
<div class="dm-modal-overlay" id="modal-rename">
  <div class="dm-modal">
    <div class="dm-modal-header">
      <div class="dm-modal-title">Ubah Nama File</div>
      <button class="dm-modal-close" onclick="closeModal('modal-rename')">&times;</button>
    </div>
    <div class="dm-modal-body">
      <label class="dm-label">Nama Baru</label>
      <input type="text" class="dm-ctrl" id="input-rename" placeholder="Nama file..." maxlength="255">
      <input type="hidden" id="rename-file-id">
      <div id="rename-msg" class="dm-msg"></div>
    </div>
    <div class="dm-modal-footer">
      <button class="dm-btn dm-btn-outline" onclick="closeModal('modal-rename')">Batal</button>
      <button class="dm-btn" style="background:#7B1C1C;color:#fff;border-color:#5A1212" onclick="submitRename()">Simpan</button>
    </div>
  </div>
</div>
...
(function(){
  ...
  function renderFileList() {
    fileList.innerHTML = pendingFiles.map((f,i) =>
      ...
      + '<span style="flex:1;color:#1C1714;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + f.name + '</span>'
  ...
  uploadBtn?.addEventListener('click', async () => {
    ...
    for (const file of pendingFiles) {
      ...
      try {
        ...
        results.push({ name: file.name, ok: data.success, msg: data.message, file: data.file });
        if (data.success && data.file) appendFileRow(data.file);
      } catch(e) {
        results.push({ name: file.name, ok: false, msg: 'Gagal koneksi' });
      }
      done++;
    ...
    if (!failed.length) {
      setMsg('upload-msg', 'Semua file berhasil diupload.', true);
      setTimeout(() => { closeModal('modal-upload'); location.reload(); }, 1200);
    } else {
      setMsg('upload-msg', failed.map(f => f.name+': '+f.msg).join('<br>'), false);
  ...
  function appendFileRow(f) {
    ...
    tr.id = 'file-row-' + f.id;
    tr.innerHTML =
      '<td><span class="dm-file-icon" style="background:'+f.mime_color+'">'+f.mime_label+'</span></td>'
      +'<td><div class="dm-file-name">'+escHtml(f.original_name)+'</div></td>'
      +'<td><span style="font-size:12px;color:#6B6055">'+f.mime_label+'</span></td>'
      +'<td class="dm-file-size">'+f.size_fmt+'</td>'
      +'<td style="font-size:12.5px;color:#6B6055"><?= htmlspecialchars(Auth::user()['name']) ?></td>'
      +'<td style="font-size:12px;color:#A89E90">'+new Date().toLocaleDateString('id-ID',{day:'2-digit',month:'short',year:'numeric'})+'</td>'
      +'<td style="text-align:right">'
      +'<a href="<?= $base ?>/dokumen/'+f.id+'/download" class="dm-btn dm-btn-outline dm-btn-sm">↓</a> '
      +'<button class="dm-btn dm-btn-danger dm-btn-sm" onclick="deleteFile('+f.id+',\''+f.original_name.replace(/'/g,"\'")+'\')">🗑</button>'
      +'</td>';
  ...
  /* ── Buat/Rename Folder ── */
  let folderMode = 'create'; // 'create' | 'rename'
  ...
  window.openFolderMenu = function(btn) {
    // simple: langsung buka rename (bisa dikembangkan ke dropdown)
    folderMode = 'rename';
  ...
  window.submitFolder = async function() {
    ...
    if (folderMode === 'create') {
      ...
      const fid = document.getElementById('folder-action-id').value;
      url = BASE + '/api/dokumen/folder/' + fid + '/rename';
    }
    try {
  ...
  window.deleteFile = function(id, name) {
    if (!confirm('Hapus file "' + name + '"? Tindakan ini tidak dapat dibatalkan.')) return;
    fetch(BASE + '/api/dokumen/' + id + '/delete', { method:'POST' })
      ...
      .then(data => {
        if (data.success) {
          ...
          setMsg('page-msg', data.message, false);
        }
      })
