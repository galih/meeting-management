...
<!-- ============================================================
     KEMENBUD DEPARTMENTS PAGE
     Desain konsisten dengan /users
============================================================ -->
<style>
:root {
  --dp-primary:        #7B1C1C;
  ...
  --dp-orange:         #C05621;
  --dp-orange-bg:      rgba(192,86,33,.10);
  --dp-radius:         12px;
  --dp-radius-sm:      8px;
  --dp-radius-xs:      6px;
...
.dp-btn-ghost { background:transparent; color:var(--dp-text-muted); border-color:transparent; }
.dp-btn-ghost:hover { background:var(--dp-primary-light); color:var(--dp-primary); }
...
.dp-toggle {
  ...
  transition:all var(--dp-transition); font-size:11px;
}
...
@media(max-width:767.98px){
  ...
  .dp-actions { flex-direction:column; gap:.2rem; }
  .dp-card-body { flex-wrap:wrap; gap:.5rem; }
}
</style>
...
?>
<div class="dp-node" id="dpnode-<?= $unit['id'] ?>">
    ...
    <div class="dp-card-body">
      ...
      <div class="dp-node-info">
        ...
        <div class="dp-node-meta">
          <span>
            <span class="dp-badge <?= $lc['badge'] ?>"><?= $lbl ?></span>
          </span>
          ...
          <?php if ($hasChildren): ?>
          <span style="color:var(--dp-text-faint);">
            <?= count($unit['children']) ?> sub-unit
          </span>
          <?php endif; ?>
        </div>
      </div>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <form method="POST" action="<?= $baseUrl ?>/departments">
          ...
          <div style="display:grid;gap:.75rem;">
            ...
            <div style="display:grid;grid-template-columns:1fr auto;gap:.75rem;align-items:start;">
              <div>
                ...
                <input type="text" name="name" class="dp-form-control" required placeholder="cth: Bidang Perencanaan">
              </div>
              <div>
        ...
        <div class="modal-footer">
          <button type="button" class="dp-btn dp-btn-ghost" data-bs-dismiss="modal">Batal</button>
          <button type="submit" class="dp-btn dp-btn-primary">Simpan Unit</button>
        </div>
      </form>
    </div>
  </div>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <form method="POST" id="formEditUnit">
        ...
        <div class="modal-footer">
          <button type="button" class="dp-btn dp-btn-ghost" data-bs-dismiss="modal">Batal</button>
          <button type="submit" class="dp-btn dp-btn-primary">Simpan Perubahan</button>
        </div>
      </form>
    </div>
  </div>
...
const deptLvNames  = <?= json_encode($levelLabel) ?>;
const deptLvBadge  = { 1:'dept-badge-red', 2:'dept-badge-blue', 3:'dept-badge-green' };

/* ── Safe Bootstrap Modal (sama seperti /users) ─────────── */
function bsModal(el) {
...
function updateLevelPreview(prefix) {
  ...
  const lbl  = deptLvNames[lv] || 'Unit';
  const cls  = deptLvBadge[lv] || 'dept-badge-red';
  prev.innerHTML = `<span class="dp-level-badge dp-badge ${cls}">Level ${lv} — ${lbl}</span>`;
}
// init preview modal tambah
...
function toggleChildren(btn, targetId) {
  ...
  btn.setAttribute('aria-label', collapsed ? 'Collapse' : 'Expand');
...
document.querySelectorAll('.btn-edit-dept').forEach(btn => {
  btn.addEventListener('click', function () {
    ...
    document.getElementById('formEditUnit').action  = deptBaseUrl + '/departments/' + d.id + '/update';
...
document.getElementById('btn-konfirmasi-hapus-dept').addEventListener('click', async () => {
  const btn = document.getElementById('btn-konfirmasi-hapus-dept');
  btn.disabled = true; btn.textContent = 'Menghapus…';
  try {
    const res = await fetch(hapusDeptUrl, { method: 'POST' });
    const d   = await res.json();
    bsModalGet(document.getElementById('modalHapusDept')).hide();
    if (d.success) {
      const node = document.getElementById('dpnode-' + hapusDeptId);
      node?.remove();
    } else {
