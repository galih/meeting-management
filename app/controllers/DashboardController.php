    ...
    public function index(): void {
        ...
        if ($user['role'] === 'admin') {
            $stats = [
                ...
                'tl_overdue'      => Database::queryOne("SELECT COUNT(*) c FROM tindak_lanjut WHERE deadline < CURDATE() AND status NOT IN ('done','cancelled')")['c'],
            ...
            $stats = [
                ...
                'tl_overdue'      => Database::queryOne("SELECT COUNT(*) c FROM tindak_lanjut WHERE assigned_to=? AND deadline < CURDATE() AND status NOT IN ('done','cancelled')", [$uid])['c'],
        ...
        $tlDeadline = ($user['role'] === 'admin')
            ? Database::query(
                "SELECT tl.*, m.title as meeting_title, u.name as assigned_name
                 ...
                 ORDER BY tl.deadline ASC LIMIT 5"
              )
            : Database::query(
                "SELECT tl.*, m.title as meeting_title, u.name as assigned_name
                 FROM tindak_lanjut tl
